...
export const features: Feature[] = [
  {
    ...
    <p class="pb-4">Fidgrove supports <a href="https://www.studio-397.com/" target="_blank" class="text-blue">rFactor 2 </a> esports platform, and all its content. Fidgrove keeps track of all version numbers of the cars & tracks, using file hashes. This makes it possible to validate if the content is different from previous versions, or if it has been tampered with (e.g., car physics). Different versions will be marked as such. In some cases, Fidgrove uses “sub-versions”, which means content with same version but slightly different (e.g., car impacted by Balance of Perfomance).</p>
